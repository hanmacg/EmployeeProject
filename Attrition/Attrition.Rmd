...
spread_ed
phd_count <- sum(employee$ExperienceInCurrentDomain == "7")
```
...
When examining the frequency of attrition, 
...
There are several hyperparamters to consider in this model: the number of trees, the number of features to consider at a given split, the complexity of each tree, the sampling scheme, the splitting rule to use during tree construction.
...
hyperparams <- list(
  num.trees = c(100, 200, 300),  # Example values, modify as needed
